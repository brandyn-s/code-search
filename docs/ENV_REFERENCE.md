...
| `CODE_SYNONYMS_PATH` | unset | Path to a JSON object overlaying the built-in `CODE_SYNONYMS` map in `search/searcher.py`. Per-key values: a list of synonyms (extends/overrides that key) or `null` (removes the built-in key — e.g., disable the Corsair daemon expansions on a non-Corsair corpus). Unset = built-in map, byte-identical behavior. Load failure logs a warning and falls back to built-ins. Changing the built-in *defaults* (removing Corsair entries from source) is a retrieval-behavior change: **BLOCKED ON MEASUREMENT** per ship-discipline rule 10. |
