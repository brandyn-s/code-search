...
Those are measured-evidence records — do not
...
|----------|---------|---------|
| `EMBEDDING_PROVIDER` | `voyage` (if `VOYAGE_API_KEY` set) | Provider: `voyage` (recommended, uses voyage-4-large), `voyage-code-3` (available non-default; TypeScript-optimized, regresses on Nix — see `internal eval finding (2026-05-15)`), `voyage-context` (legacy contextualized), `openai`, `jina` (local, code-optimized), `local` |
...
| `CODE_SYNONYM_PROFILE` | `generic` | Select the built-in synonym profile: `generic`, `corsair`, or `off`. `generic` covers common programming vocabulary; `corsair` is a domain-specific profile (Nix services, telemetry, navigation terms) kept for existing deployments. `off` disables expansion. |
