# Listwise Reranker — Selectable Mode (Default Flip REVERTED 2026-05-23)

**Status (2026-05-23 post-revert): NON-DEFAULT.** Listwise reranker is selectable via `RERANKER=listwise` but is **not** the production default. PR #199 (2026-05-23) flipped the default citing Phase C v2 bootstrap CI; that evidence was stale per ship-discipline rule 9 (R9 / PR #193 modified pointwise's `JUDGE_PROMPT` between Phase C v2 and PR #199). The rule-9 re-eval on current main showed listwise harvested MRR delta −0.0456 CI [−0.0891, −0.0024] and real_session_v1 (n=148) delta −0.0622 CI [−0.108, −0.017] — both CIs exclude zero unfavorable, ship-gate matrix REVERT row fires. Default flipped back to pointwise (`RERANKER=sonnet`) the same day. See `docs/findings/2026-05-23-listwise-default-eval-finding.md` for the full narrative.

> History: the 2026-05-16 canary phase shipped listwise as opt-in per [GPT-5.5-pro pass 5 — Phase C v2 ship-readiness](https://github.com/redacted-org/claude-knowledge-base/pull/554). PR #199 briefly flipped the default to listwise on Phase C v2's +0.044 MRR harvested CI — but Phase C v2 measured against PRE-R9 pointwise. R9 (PR #193) lifted pointwise's harvested MRR enough that the post-R9 comparison inverts: listwise is now net-negative on the production-query distribution. The latency justification (1 call vs slowest-of-15) was never measured on this codebase and remains separately BLOCKED ON MEASUREMENT per ship-discipline rule 10.
