...
**Reranking.** The current default is **Sonnet 4.6 listwise reranking** (`RERANKER=listwise`, graduated from opt-in canary on 2026-05-23 per Phase C v2 bootstrap CI: +0.044 MRR CI [+0.003, +0.084] harvested, +0.047 nDCG@10 CI [+0.004, +0.095] golden — see `CLAUDE.md` and `docs/LISTWISE_CANARY.md`). A single comparative call ranks the top-15 hybrid candidates, replacing the pointwise reranker's 15 isolated per-candidate calls. Always-on graceful fallback to hybrid order on missing key / timeout / parse failure. The previous pointwise default (`RERANKER=sonnet`, validated +0.087 MRR / +0.137 HR@1 on n=183 multi-target real_session) is retained as the regression-observation fallback. The earlier "cross-encoder rerank (rerank-2.5) degrades quality by -30% MRR" finding still holds and is preserved as the off-by-default `RERANKER=cross-encoder` legacy path; `RERANKER=off` skips reranking entirely.
