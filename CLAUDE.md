...
| `SONNET_RERANKER_HYBRID_PRIOR_THRESHOLD` | `7` | Hybrid-prior fallback threshold (added 2026-05-03 via A0 investigation). When the max Sonnet score across the candidate pool is below this value, the reranker is uncertain — score ties get arbitrary tie-breaking that favors keyword-dense chunks over canonical files. Falls back to hybrid order in that case. Set to `0` to disable. Validated +0.011 HR@5 / +0.016 HR@1 / +0.007 MRR on n=183 simulation (PR #95+). |
